...
pub async fn post_generate_lesson_plan(
    ...
    Ok(Json(lesson_plan_ai::generate_plan(&state.db, state.ai_provider.as_ref(), model, &ctx, body).await?))
}
...
pub async fn post_edit_lesson_section(
    ...
    Ok(Json(lesson_plan_ai::edit_section(&state.db, state.ai_provider.as_ref(), &state.config.ai_lesson_generation_model, &ctx, body).await?))
}
...
pub async fn post_translate_lesson_plan(
    ...
    Ok(Json(lesson_plan_ai::translate_plan(&state.db, state.ai_provider.as_ref(), &state.config.ai_lesson_generation_model, &ctx, body).await?))
}
