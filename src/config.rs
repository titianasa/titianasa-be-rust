...
pub struct Config {
    ...
    // GCP migration — Vertex AI Gemini backs `AppState::text_ai_provider`
    // (pure text generation only; OCR/STT/TTS stay on `ai_provider`/
...
impl Config {
    pub fn from_env() -> anyhow::Result<Self> {
        Ok(Self {
            ...
            ai_writing_evaluation_model: std::env::var("AI_WRITING_EVALUATION_MODEL").unwrap_or_else(|_| "gemini-3.8-flash".into()),
            ai_speaking_evaluation_model: std::env::var("AI_SPEAKING_EVALUATION_MODEL").unwrap_or_else(|_| "gemini-3.8-flash".into()),
            ai_grammar_evaluation_credit_cost: env_int("AI_GRAMMAR_EVALUATION_CREDIT_COST", 1)?,
            ai_grammar_evaluation_model: std::env::var("AI_GRAMMAR_EVALUATION_MODEL").unwrap_or_else(|_| "gemini-3.8-flash".into()),
            ai_lesson_generation_model: std::env::var("AI_LESSON_GENERATION_MODEL").unwrap_or_else(|_| "gemini-3.8-flash".into()),
            ai_question_generation_model: std::env::var("AI_QUESTION_GENERATION_MODEL").unwrap_or_else(|_| "gemini-3.8-flash".into()),
            ai_ocr_model: std::env::var("AI_OCR_MODEL").unwrap_or_else(|_| "gemini-3.8-flash".into()),
